// Asigna video_path (video corto) a ejercicios — Batch 003 / GOAT 25/04/2026
...
export async function updateVideosBatch003(
  ...
  for (const pair of BATCH_003_VIDEO_PAIRS) {
    ...
    if (!exerciseId) {
      console.warn(`[VideosBatch003] No encontrado: "${pair.exerciseName}"`);
      skippedNoExercise++;
      continue;
    }
    ...
    console.log(`[VideosBatch003] OK: "${pair.exerciseName}" → ${videoUrl}`);
    updated++;
  }
