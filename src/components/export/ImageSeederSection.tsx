...
function isScapularDone() {
  return localStorage.getItem('images_scapular_push_up_done_v1') === 'true';
}

async function runScapularUpdate() {
  const { updateImagesScapularPushUp } = await import(
    '../../services/imagesScapularPushUpUpdateService'
  );
  return updateImagesScapularPushUp();
}

const IMAGE_ENTRIES: ImageEntry[] = [
  {
    label: 'Scapular Push-Up — imagen',
    description: 'Cuadrupedia, perro malo / perro bueno',
    isDone: isScapularDone,
    run: runScapularUpdate,
  },
];
    ...
    try {
      ...
      if (result.updated > 0) {
        ...
        if (result.skippedNoExercise > 0)
          msg += ` · ${result.skippedNoExercise} ejercicios no encontrados`;
        toast.success(msg);
