// Selección interactiva de videos cortos — Batch 003 / GOAT 25/04/2026
...
import { BATCH_003_VIDEO_PAIRS, ExerciseVideoPair } from '../../data/batch003VideoPairs';
...
export function VideoBatch003Page() {
  ...
  const total = BATCH_003_VIDEO_PAIRS.length;

  const filtered = filter
    ? BATCH_003_VIDEO_PAIRS.filter(p => p.exerciseName.toLowerCase().includes(filter.toLowerCase()))
  ...
  const handleSelectAll = (choice: 'A' | 'B') => {
    ...
    BATCH_003_VIDEO_PAIRS.forEach(p => { all[p.exerciseName] = choice; });
    setSelections(all);
  };
    ...
    <>
      <div className="sticky top-0 z-30 bg-gray-950 border-b border-gray-800 flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1">
          <h1 className="text-sm font-bold text-white leading-tight">Videos cortos — GOAT 25/04</h1>
          <p className="text-[11px] text-gray-500">Asignar video_path · 6 ejercicios</p>
        </div>
