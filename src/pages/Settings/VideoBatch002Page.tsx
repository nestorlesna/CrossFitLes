// Selección interactiva de videos explicativos — Batch 002 / GOAT 22/04/2026
import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { ArrowLeft, Play, Save, Loader2 } from 'lucide-react';
import { toast } from 'sonner';
import { updateVideosBatch002 } from '../../services/videosBatch002UpdateService';
import { BATCH_002_VIDEO_PAIRS, ExerciseVideoPair } from '../../data/batch002VideoPairs';
...
export function VideoBatch002Page() {
  ...
  const total = BATCH_002_VIDEO_PAIRS.length;

  const filtered = filter
    ? BATCH_002_VIDEO_PAIRS.filter(p =>
        p.exerciseName.toLowerCase().includes(filter.toLowerCase())
      )
    : BATCH_002_VIDEO_PAIRS;
  ...
  const handleSelectAll = (choice: 'A' | 'B') => {
    ...
    BATCH_002_VIDEO_PAIRS.forEach(p => { all[p.exerciseName] = choice; });
    setSelections(all);
  };
  ...
  const handleApply = async () => {
    ...
    setLoading(true);
    try {
      const result = await updateVideosBatch002(selections);
    ...
    <>
      ...
      <div className="sticky top-0 z-30 bg-gray-950 border-b border-gray-800 flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1">
          <h1 className="text-sm font-bold text-white leading-tight">Videos explicativos — GOAT 22/04</h1>
          <p className="text-[11px] text-gray-500">Asignar video_long_path</p>
        </div>
