// Selección interactiva de videos explicativos — Batch 004 / GOAT 25/04/2026
import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { ArrowLeft, Play, Save, Loader2 } from 'lucide-react';
import { toast } from 'sonner';
import { updateVideosBatch004 } from '../../services/videosBatch004UpdateService';
import { BATCH_004_VIDEO_PAIRS, ExerciseVideoPair } from '../../data/batch004VideoPairs';
        ...
        {(['A', 'B'] as const).map((choice) => {
          ...
          return (
            <div key={choice} className="relative">
              ...
              >
                {thumb ? (
                  <img src={thumb} alt={`Video ${choice}`} className="w-full aspect-video object-cover" />
                ) : (
...
export function VideoBatch004Page() {
  ...
  const total = BATCH_004_VIDEO_PAIRS.length;

  const filtered = filter
    ? BATCH_004_VIDEO_PAIRS.filter(p => p.exerciseName.toLowerCase().includes(filter.toLowerCase()))
    : BATCH_004_VIDEO_PAIRS;
  ...
  const handleSelectAll = (choice: 'A' | 'B') => {
    ...
    BATCH_004_VIDEO_PAIRS.forEach(p => { all[p.exerciseName] = choice; });
    setSelections(all);
  };
  ...
  const handleApply = async () => {
    ...
    setLoading(true);
    try {
      const result = await updateVideosBatch004(selections);
      let msg = `${result.updated} video${result.updated !== 1 ? 's' : ''} explicativo${result.updated !== 1 ? 's' : ''} asignado${result.updated !== 1 ? 's' : ''}`;
    ...
    <>
      <div className="sticky top-0 z-30 bg-gray-950 border-b border-gray-800 flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1">
          <h1 className="text-sm font-bold text-white leading-tight">Videos explicativos — GOAT 25/04</h1>
          <p className="text-[11px] text-gray-500">Asignar video_long_path · 3 ejercicios</p>
        </div>
      ...
      <Modal isOpen={!!previewUrl} onClose={() => setPreviewUrl(null)} title="Preview de video" size="md">
        {previewUrl && (
